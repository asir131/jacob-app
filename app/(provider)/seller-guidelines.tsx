import { Ionicons } from "@expo/vector-icons";
import { useRouter } from "expo-router";
import React from 'react';
import { ScrollView, Text, TouchableOpacity, View } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";

export default function SellerGuidelinesPage() {
    const router = useRouter();

    const GuidelineSection = ({ title, icon, children }: { title: string; icon: string; children: React.ReactNode }) => (
        <View className="mb-8">
            <View className="flex-row items-center mb-4">
                <Ionicons name={icon as any} size={24} color="#2B84B1" />
                <Text className="text-[18px] font-bold text-[#1A2C42] ml-3">{title}</Text>
            </View>
            <View className="bg-white rounded-[20px] px-5 py-4 border border-gray-100 shadow-sm shadow-gray-100">
                {children}
            </View>
        </View>
    );

    const GuidelineItem = ({ title, description }: { title: string; description: string }) => (
        <View className="mb-4">
            <Text className="text-[16px] font-semibold text-[#1A2C42] mb-2">{title}</Text>
            <Text className="text-[14px] text-[#7C8B95] leading-6">{description}</Text>
        </View>
    );

    const ChecklistItem = ({ text }: { text: string }) => (
        <View className="flex-row items-start mb-3">
            <Ionicons name="checkmark-circle" size={16} color="#10B981" className="mt-0.5 mr-3" />
            <Text className="text-[14px] text-[#1A2C42] flex-1 leading-5">{text}</Text>
        </View>
    );

    return (
        <SafeAreaView className="flex-1 bg-[#FAFCFD]" edges={['top']}>
            {/* Header */}
            <View className="px-6 py-4 flex-row items-center bg-white shadow-sm shadow-black/5 z-10">
                <TouchableOpacity onPress={() => router.back()} className="w-10 h-10 items-center justify-center bg-gray-50 rounded-full mr-4">
                    <Ionicons name="arrow-back" size={20} color="#1A2C42" />
                </TouchableOpacity>
                <Text className="text-[20px] font-bold text-[#1A2C42]">Seller Guidelines</Text>
            </View>

            <ScrollView className="flex-1 px-6 pt-6" showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100 }}>

                {/* Introduction */}
                <View className="bg-gradient-to-r from-[#2B84B1] to-[#3B93C1] rounded-[24px] px-6 py-6 mb-8">
                    <Text className="text-[20px] font-bold mb-2">Welcome to Our Platform</Text>
                    <Text className="text-[14px] leading-6">
                        These guidelines help you provide excellent service to buyers and maintain a successful seller profile.
                        Following these best practices ensures a positive experience for everyone.
                    </Text>
                </View>

                {/* Getting Started */}
                <GuidelineSection title="Getting Started" icon="rocket-outline">
                    <GuidelineItem
                        title="Account Setup"
                        description="Complete your profile with accurate information, professional photo, and detailed description of your skills and experience."
                    />
                    <GuidelineItem
                        title="Verification"
                        description="Verify your account with valid identification and contact information to build trust with buyers."
                    />
                    <GuidelineItem
                        title="Profile Optimization"
                        description="Use relevant keywords in your profile and service descriptions to improve discoverability."
                    />
                </GuidelineSection>

                {/* Creating Services */}
                <GuidelineSection title="Creating Services" icon="create-outline">
                    <GuidelineItem
                        title="Clear Service Titles"
                        description="Use descriptive, keyword-rich titles that clearly explain what you're offering."
                    />
                    <GuidelineItem
                        title="Detailed Descriptions"
                        description="Provide comprehensive descriptions including what's included, delivery time, and any requirements."
                    />
                    <GuidelineItem
                        title="Pricing Strategy"
                        description="Set competitive prices based on your experience level and market rates. Consider offering packages."
                    />
                    <GuidelineItem
                        title="High-Quality Media"
                        description="Use professional images, screenshots, or examples to showcase your work and attract buyers."
                    />
                </GuidelineSection>

                {/* Communication */}
                <GuidelineSection title="Communication" icon="chatbubbles-outline">
                    <GuidelineItem
                        title="Response Time"
                        description="Respond to buyer inquiries within 24 hours. Faster responses improve your chances of getting hired."
                    />
                    <GuidelineItem
                        title="Professional Tone"
                        description="Maintain a professional, friendly tone in all communications. Be clear and courteous."
                    />
                    <GuidelineItem
                        title="Requirement Clarification"
                        description="Ask detailed questions to understand buyer needs before starting work."
                    />
                    <GuidelineItem
                        title="Regular Updates"
                        description="Keep buyers informed about progress and any potential delays."
                    />
                </GuidelineSection>

                {/* Delivery & Quality */}
                <GuidelineSection title="Delivery & Quality" icon="checkmark-circle-outline">
                    <GuidelineItem
                        title="Meeting Deadlines"
                        description="Deliver work on or before the agreed delivery date. Communicate if delays are unavoidable."
                    />
                    <GuidelineItem
                        title="Quality Standards"
                        description="Ensure all deliverables meet professional standards and match the service description."
                    />
                    <GuidelineItem
                        title="Revisions Policy"
                        description="Offer reasonable revisions as part of your service. Clearly communicate your revision policy."
                    />
                    <GuidelineItem
                        title="File Delivery"
                        description="Provide files in appropriate formats and include any necessary instructions or documentation."
                    />
                </GuidelineSection>

                {/* Best Practices */}
                <GuidelineSection title="Best Practices" icon="star-outline">
                    <Text className="text-[16px] font-semibold text-[#1A2C42] mb-4">Do's and Don'ts</Text>

                    <View className="mb-6">
                        <Text className="text-[15px] font-medium text-[#1A2C42] mb-3">✅ Do's</Text>
                        <ChecklistItem text="Deliver high-quality work that exceeds expectations" />
                        <ChecklistItem text="Communicate clearly and professionally" />
                        <ChecklistItem text="Meet agreed deadlines and delivery times" />
                        <ChecklistItem text="Provide excellent customer service" />
                        <ChecklistItem text="Keep your profile and services updated" />
                        <ChecklistItem text="Offer fair pricing for your expertise" />
                    </View>

                    <View>
                        <Text className="text-[15px] font-medium text-[#1A2C42] mb-3">❌ Don'ts</Text>
                        <ChecklistItem text="Misrepresent your skills or experience" />
                        <ChecklistItem text="Overpromise and underdeliver" />
                        <ChecklistItem text="Ignore buyer communications" />
                        <ChecklistItem text="Use copyrighted materials without permission" />
                        <ChecklistItem text="Engage in unethical business practices" />
                        <ChecklistItem text="Spam or harass buyers" />
                    </View>
                </GuidelineSection>

                {/* Dispute Resolution */}
                <GuidelineSection title="Dispute Resolution" icon="shield-checkmark-outline">
                    <GuidelineItem
                        title="Handling Issues"
                        description="Address buyer concerns promptly and professionally. Most issues can be resolved through clear communication."
                    />
                    <GuidelineItem
                        title="Escalation Process"
                        description="If you cannot resolve an issue directly, contact our support team for mediation."
                    />
                    <GuidelineItem
                        title="Prevention"
                        description="Set clear expectations upfront and maintain detailed communication throughout the project."
                    />
                </GuidelineSection>

                {/* Account Management */}
                <GuidelineSection title="Account Management" icon="settings-outline">
                    <GuidelineItem
                        title="Profile Maintenance"
                        description="Keep your profile information current and respond to reviews and feedback."
                    />
                    <GuidelineItem
                        title="Performance Monitoring"
                        description="Track your response times, completion rates, and buyer satisfaction scores."
                    />
                    <GuidelineItem
                        title="Account Security"
                        description="Use strong passwords, enable two-factor authentication, and protect your account information."
                    />
                </GuidelineSection>

                {/* Need Help? */}
                <View className="bg-[#F0F8FF] rounded-[20px] px-6 py-6 mb-8 border border-[#2B84B1]/20">
                    <View className="flex-row items-center mb-3">
                        <Ionicons name="help-circle-outline" size={24} color="#2B84B1" />
                        <Text className="text-[18px] font-bold text-[#1A2C42] ml-3">Need Help?</Text>
                    </View>
                    <Text className="text-[14px] text-[#7C8B95] mb-4 leading-6">
                        If you have questions about these guidelines or need assistance with your seller account,
                        our support team is here to help.
                    </Text>
                    <TouchableOpacity className="bg-[#2B84B1] py-3 px-6 rounded-[12px] items-center shadow-sm shadow-[#2B84B1]/40">
                        <Text className="text-white font-semibold">Contact Support</Text>
                    </TouchableOpacity>
                </View>

            </ScrollView>
        </SafeAreaView>
    );
}
