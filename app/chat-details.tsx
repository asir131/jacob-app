    ...
    <View className="flex-1">
      ...
      >
        ...
        >
          ...
          <Text className="text-[#2286BE] font-bold text-[18px] ml-2">
            Go Back
          </Text>
        </TouchableOpacity>
        ...
        <View className="flex-row items-center justify-between">
          <View className="flex-row items-center">
            <Image
              source={{
                uri: (avatar as string) || "https://i.pravatar.cc/150?u=john",
              }}
              className="w-[52px] h-[52px] rounded-full mr-4"
            />
            <View>
              <Text className="text-[20px] font-bold text-[#2286BE]">
                {name}
              </Text>
              <Text className="text-[14px] font-medium text-[#7C8B95] mt-1">
                {info}
              </Text>
            </View>
          </View>
            ...
            >
              {!item.isMe && (
                <Image
                  source={{
                    uri:
                      (avatar as string) || "https://i.pravatar.cc/150?u=john",
                  }}
              ...
              <View className={`max-w-[75%]`}>
                ...
                >
                  <Text
                    className={`text-[15px] leading-[22px] font-medium ${item.isMe ? "text-white" : "text-[#4A5568]"}`}
                  >
                    {item.text}
                  </Text>
                  {item.isMe && (
                    <Text className="text-white/70 text-[12px] mt-2 font-medium">
                      {item.time}
                    </Text>
                  )}
                </View>
                {!item.isMe && (
                  <Text className="text-[#7C8B95] text-[13px] mt-2 font-medium ml-1">
                    {item.time}
                  </Text>
                )}
              </View>
