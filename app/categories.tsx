...
export default function CategoriesPage() {
  ...
  useEffect(() => {
    const loadCategories = async () => {
      try {
        const payload = await mobileApi.getCategories();
                  ...
                  <View key={item.id} className="w-[33.33%] items-center mb-12">
                    <TouchableOpacity
                      onPress={() =>
                        router.push({
                          pathname: "/services",
                          params: { title: item.name, categorySlug: item.slug },
                        })
                      }
                    ...
                    >
                      {icon.library === "ion" ? (
                        <Ionicons name={icon.name as any} size={32} color={icon.iconColor} />
                      ) : (
                        <MaterialCommunityIcons name={icon.name as any} size={32} color={icon.iconColor} />
                      )}
                    </TouchableOpacity>
